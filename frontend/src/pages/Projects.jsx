...
function AllocateModal({ project, onClose, onAllocated, token }) {
  ...
  useEffect(() => {
    axios.get(`${API}/resources`, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => {
        // Show only available resources (not already running elsewhere)
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#111827] border border-white/10 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="px-6 py-4 max-h-[60vh] overflow-y-auto">
              ...
              {resources.map(res => (
                <button
                  ...
                  className={`w-full text-left p-4 rounded-xl border transition-all ${
                    selected === res.id
                      ? 'border-indigo-500 bg-indigo-500/10 shadow-[0_0_0_1px_rgba(99,102,241,0.3)]'
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">{res.region}</p>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      ...
                      <span className="inline-block mt-1 bg-zinc-700 text-zinc-300 text-xs px-1.5 py-0.5 rounded">{res.type}</span>
                    </div>
                  </div>
          ...
          {error && (
            <div className="mt-4 flex items-center gap-2 bg-rose-500/10 border border-rose-500/20 text-rose-400 rounded-lg px-4 py-3 text-sm">
        ...
        <div className="px-6 py-4 border-t border-white/10 flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 rounded-lg border border-white/10 text-gray-300 hover:bg-white/5 text-sm font-medium transition-colors">
            Cancel
          </button>
          <button
            onClick={handleAllocate}
            disabled={!selected || saving}
            className="px-5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-semibold transition-colors flex items-center gap-2"
          >
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white placeholder-gray-600 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition"
            />
          </div>
          <div>
            ...
            <textarea
              ...
              className="w-full px-3 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white placeholder-gray-600 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition resize-none"
            />
          </div>
          {error && (
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose} className="px-4 py-2 rounded-lg border border-white/10 text-gray-300 hover:bg-white/5 text-sm font-medium transition-colors">
              Cancel
            </button>
            <button type="submit" disabled={saving || !name.trim()} className="px-5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-sm font-semibold transition-colors flex items-center gap-2">
...
export default function Projects() {
  ...
  const [allocateTarget, setAllocateTarget] = useState(null); // project to allocate to
  const [terminating, setTerminating]    = useState(null); // allocationId being terminated
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="px-4 py-2 rounded-lg border border-indigo-500/30 bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500/20 font-semibold text-sm transition-all flex items-center gap-2 disabled:opacity-50"
          >
          ...
          <button
            ...
            className="px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-sm transition-all flex items-center gap-2 shadow-lg shadow-indigo-500/20"
          >
      ...
      {invoiceMsg && (
        <div className={`flex items-start gap-3 px-4 py-3 rounded-xl border text-sm font-medium ${
          ...
          <button onClick={() => setInvoiceMsg('')} className="ml-auto shrink-0 opacity-60 hover:opacity-100">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
      ...
      {/* Empty state */}
      {projects.length === 0 && (
        <div className="col-span-full flex flex-col items-center justify-center py-24 bg-card rounded-2xl border border-border">
          ...
          <button onClick={() => setShowCreate(true)} className="px-5 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-sm flex items-center gap-2 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {projects.map(project => {
          ...
          return (
            <div key={project.id} className="bg-card border border-border rounded-2xl overflow-hidden flex flex-col hover:border-indigo-500/30 transition-all group">
              {/* Card Header */}
              <div className="px-6 pt-6 pb-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button
                    ...
                    className="p-2 rounded-lg text-muted-foreground hover:bg-rose-500/10 hover:text-rose-400 transition-colors shrink-0"
                ...
                <div className="flex items-center gap-4 mt-4 pt-4 border-t border-border">
                  ...
                  <div className="ml-auto text-right">
                    ...
                    <p className="text-base font-bold text-indigo-400">₹{monthlyCost.toFixed(2)}<span className="text-xs text-muted-foreground font-normal">/mo</span></p>
                  </div>
                </div>
              </div>
              ...
              <div className="px-6 py-4 border-t border-border flex-1">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs font-semibold text-indigo-400 hover:text-indigo-300 transition-colors px-2.5 py-1 rounded-lg hover:bg-indigo-500/10"
                  >
                ...
                <div className="space-y-2">
                  {activeAllocs.map(alloc => (
                    <div key={alloc.id} className="flex items-center gap-3 p-3 rounded-xl bg-muted/50 border border-border group/alloc">
                      ...
                      <div className="text-right shrink-0">
                        <p className="text-xs font-semibold text-emerald-400">₹{alloc.resource.baseCost.toFixed(2)}/mo</p>
                      </div>
                      <button
                        onClick={() => handleTerminate(alloc.id)}
                        disabled={terminating === alloc.id}
                        title="Deallocate resource"
                        className="opacity-0 group-hover/alloc:opacity-100 ml-1 p-1.5 rounded-lg text-muted-foreground hover:bg-rose-500/10 hover:text-rose-400 transition-all disabled:cursor-not-allowed"
                      >
                  ...
                  <details className="mt-3">
                    ...
                    <div className="space-y-1.5 mt-2">
                      {terminatedAllocs.map(alloc => (
                        <div key={alloc.id} className="flex items-center gap-3 p-2.5 rounded-lg bg-muted/30 opacity-60">
                          <div className="p-1 rounded bg-background">{resourceIcon(alloc.resource.type)}</div>
                          <div className="flex-1 min-w-0">
